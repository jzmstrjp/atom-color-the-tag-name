...
export default {
	...
	notify() {
		...
		var dataAttrName = View.querySelectorAll('[data-attr-name]');
		var dataPropName = View.querySelectorAll('[data-prop-name]');
		var cmTag = View.querySelectorAll('.syntax--tag:not(.syntax--meta)');
		var cmAttr = View.querySelectorAll('.syntax--attribute-name');
		var cmProp = View.querySelectorAll('.syntax--property-name');
		Array.prototype.forEach.call(dataTagName, function(elm, i, arr) {
			elm.removeAttribute("data-tag-name");
		});
		Array.prototype.forEach.call(dataAttrName, function(elm, i, arr) {
			elm.removeAttribute("data-attr-name");
		});
		Array.prototype.forEach.call(dataPropName, function(elm, i, arr) {
			elm.removeAttribute("data-prop-name");
		});
